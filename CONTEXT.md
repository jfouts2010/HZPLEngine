...
For an **airport**, inherited building level determines the size of its abstract runway system rather than serving as runway hit points. Build levels 1–5 provide one runway-capacity channel and build levels 6–10 provide two. Each channel stores a separate runway-damage level from zero through five. Damage level zero is operational; the first point represents a mission-killing crater and closes that channel. Further direct hits deepen damage for eventual repair-time purposes, capped at five, but repair itself remains deferred. Thus a two-channel airport closes only when both channels have at least one damage point.
...
_Avoid_: treating a capacity channel as a physical runway, persisting individual crater positions, using area effects to damage several runway aim points, persisting a competing airport schedule, or adding runway headings, taxi queues, parking, weather limits, and aircraft-specific runway requirements before a rule needs them.
...
Operational runway channels affect package timing, launch eligibility, recovery selection, and diversion. A friendly airport with no operational channel is closed but not overrun: grounded aircraft remain present while unlaunched packages cancel or abort, and airborne flights use the normal recovery-airport fallback hierarchy. Runway damage is capped per channel, but airport repair remains deferred.
...
In v1, air-planning intelligence provides exact friendly squadron readiness and airfield state. Hostile airport information is projected from the observing alliance's intelligence picture through deliberately limited, airport-level reports: the airport's broad damage condition, observed runway-channel damage, information quality, observation time, and aircraft counts grouped by observed aircraft type. A report distinguishes aircraft observed on the ground from aircraft that appear available, but it does not expose hostile squadron identity, individual aircraft status, assignment, mission, package, route, or loadout.
...
These four requests form the deferred autonomous air-tasking backbone. OCA counter-air calculations and autonomous target-attack production remain pending their planned rewrite; scripted DEAD and OCA airport-strike packages exercise the current execution behavior directly.

**Offensive counter-air airport strike** is a scripted Strike package whose locked operational target is one known hostile airport. Its required strike flights receive explicit runway-denial, aircraft-on-ground, or airbase-facility assignments. The mission neither retargets to another airport after takeoff nor infers targets outside its fixed mission area.

**Runway-denial assignment** seeks an authored damage level on every abstract runway channel. Opportunity production interleaves channels by damage depth, so all open channels receive a denial attempt before any already-closed channel receives deeper damage. Every runway aim point requires one direct weapon release; multi-target and area-effect weapon coverage cannot turn one hit into several runway craters. A pass may still release enough separate stores to close or deepen several channels.

**Aircraft-on-ground assignment** uses the shared ground-attack opportunity system rather than a separate ramp-attack resolver. Each attack window may expose zero, one, or several individual grounded aircraft from squadrons based at the target airport. Several stores may attack several exposed aircraft, and an area-capable store may affect additional compatible aircraft in that same transient opportunity. Aircraft already airborne are never ramp targets. Destroying committed but unlaunched aircraft removes them from their assigned flight; a flight with no surviving assigned aircraft fails before takeoff.

**Airbase-facility assignment** attacks only building IDs explicitly authorized in the Strike mission plan and located in the airport's tile. Neighboring buildings do not become targets merely because they share the tile, and runway or facility hits do not automatically damage parked aircraft.

**OCA airport-strike result** is assessed per strike flight's explicit assignment. Runway denial succeeds when every channel reaches the desired damage depth, an aircraft-on-ground assignment succeeds when no target aircraft remain grounded at the airport, and a facility assignment succeeds when no authorized facility remains functional. Flights reassess after each resolved employment, stop when their objective is complete or no mission-useful ordnance remains, and otherwise retry opportunities during the bounded effect window. A target that becomes non-hostile, leaves the fixed area, or is no longer known invalidates the mission; a committed package cancels, an active flight egresses without retargeting, and already released effects still resolve.
...
**Provisional DEAD corridor completion** occurs when the deferred autonomous planner can find an acceptable ingress-and-egress route to the selected hostile airport without another DEAD attack. Scripted OCA strike execution does not restore autonomous exploitation of that corridor: until autonomous airport-strike planning returns, the planner concept is satisfied that the corridor is open, creates no placeholder strike request, and continues searching for another relevant airport corridor to open. When no relevant blocked corridor remains, it creates no DEAD request.
...
**SEAD escort** is an attached supporting flight that protects its package during ingress, action, and initial egress through contested airspace. It is not a separate package-level operation and does not lock one SAM site as a target. The escort maintains a forward screen, reacts to relevant emitters as they appear, and resumes its assigned route toward safe release when its named protected flights begin recovery.
...
**SEAD protection** is package-level permission to operate inside known SAM engagement envelopes while at least one attached SEAD escort remains airborne, released with the package, tactically available, and able to employ anti-radiation ordnance. An active preparation or unresolved anti-radiation effect also counts as continuing capability. Every active airborne flight in the package receives this permission, including the SEAD escort. Known sites still inform emitter detection and weapon employment, but entry permission does not reserve one weapon per site or require a missile already in flight. This abstracts the package commander's trust that the SEAD element is handling both plotted and unanticipated emitters. It does not make aircraft immune to SAM attack or cancel normal defensive reactions to a weapon already in flight.

SEAD protection ends when no attached SEAD escort can continue the task, including loss of the flight, recovery, loss of all aircraft, or exhaustion of anti-radiation capability after active effects resolve. If a named protected flight is still on mission when that happens, the package aborts. Flights without active package SEAD protection continue to treat known SAM envelopes as route and engagement constraints.
...
_Avoid_: making package entry depend on assigning one anti-radiation weapon to every known SAM along the route; unknown emitters are part of the reason the package carries a SEAD element.
