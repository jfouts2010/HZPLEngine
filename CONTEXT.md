...
**WVR decision range** — the eight-kilometer close-combat range within which a flight explicitly decides whether to continue toward a merge or extend. A valid firing opportunity is always evaluated first. An authorized BARCAP defender may continue with an infrared missile or aircraft-effective gun; an OCA sweep may continue only after its radar-guided air-to-air weapons are depleted and a close-range weapon remains. A flight extends when its own unresolved air-to-air effect is already pending against the target, no WVR-capable weapon remains, or its mission does not authorize a discretionary merge. Continuing reuses the existing intercept and press guidance rather than introducing a separate dogfight state or maneuver model.
