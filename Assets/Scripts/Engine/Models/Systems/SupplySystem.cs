...
namespace Engine.Models
{
    public class SupplySystem
    {
        ...
        private Dictionary<Guid, float> supplyRatioByDivisionId = new Dictionary<Guid, float>();
        ...
        public IReadOnlyDictionary<Guid, float> SupplyRatioByDivisionId => supplyRatioByDivisionId;

        public void GameTurn()
        {
            supplyRatioByDivisionId = CalculateSupplyRatios();
        }

        public float GetSupplyRatio(Guid divisionId)
        {
            return supplyRatioByDivisionId.TryGetValue(divisionId, out var supplyRatio)
                ? supplyRatio
                : 0f;
        }
        ...
        private Dictionary<Guid, float> CalculateSupplyRatios()
        {
            ...
            foreach (var hubAssignments in assignmentsByHubId.Values)
            {
                ...
                var allocationRatio = totalDemand <= 0f
                    ? 1f
                    : Mathf.Clamp01(hubSupply / totalDemand);

                foreach (var assignment in hubAssignments)
                    ratios[assignment.Division.DivisionId] = Mathf.Clamp01(allocationRatio * assignment.Falloff);
            }
